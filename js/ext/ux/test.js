...
Ext.extend(Ext.tree.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
	...
	renderElements : function(n, a, targetNode, bulkRender) {
		...
		if (n.attributes.resources != "") {
			...
			for (var i = 0; i < resourcesStroe.getCount(); i++) {
				...
				if (record.get("checked")) {
					resourcesStr += "checked=true/>" + record.get("name");
				} else {
		...
		var buf = [
				...
				(a.iconCls ? " " + a.iconCls : ""),
				'" unselectable="on" />',
				cb ? ('<input class="x-tree-node-cb" type="checkbox" name="'
						+ n.id + '" ' + (a.checked
						? 'checked="checked" />'
						: '/>')) : '',
...
/**
 * 获取数据并保存 获得的数据有 roleId 所有选中的子菜单的id 所有选中的资源id
 */
