JsonTree = Ext.extend(Ext.tree.TreePanel, {
	autoScroll : true,
	...
	initComponent : function() {
		...
		JsonTree.superclass.initComponent.call(this);
	...
	buildRoot : function() {
		var root = new Ext.tree.AsyncTreeNode({
					...
					loader : new Ext.tree.TreeLoader({
								dataUrl : this.urlGetAllTree, // 调用controller的方法，加载树的数据项
								listeners : {
	...
	buildEditor : function() {
		...
		treeEditor.on('complete', function(editor, value, startValue) {
					...
					Ext.Ajax.request('POST', this.urlInsertTree, {
								success : doSuccess,
								failure : hide
			...
			this.on('nodedrop', function(e) {
						...
						Ext.Ajax.request('POST', this.urlInsertTree, {
									...
									failure : hide
										...
										+ encodeURIComponent(Ext.encode(item)));
					});
		} else {
			this.on('nodedrop', function(e) {
						var n = e.dropNode;
		...
		if (n == null) {
			...
			Ext.Msg.confirm("提示", "是否确定删除？", function(btn, text) {
						if (btn == 'yes') {
							...
							this.el.mask('提交数据，请稍候...', 'x-mask-loading');
							// var hide =
							// this.treePanel.el.unmask.createDelegate(this.treePanel.el);
							var hide = function() {
								this.el.unmask(this.el);
								n.parentNode.removeChild(n);
							}.createDelegate(this);
							Ext.Ajax.request('POST', this.urlRemoveTree, {
										success : hide,
										failure : hide
									}, 'id=' + n.id);
						}
	...
	save : function() {
		...
		Ext.Ajax.request('POST', this.urlSortTree, {
					success : hide,
					failure : hide
	...
	prepareContext : function(node, e) {
		...
		if (this.contextMenu.items.get('remove')) {
			this.contextMenu.items.get('remove')[node.attributes.allowDelete
					? 'enable'
					: 'disable']();
		}
		if (this.contextMenu.items.get('updateNode')) {
			this.contextMenu.items.get('updateNode')[node.attributes.allowEdit
					? 'enable'
					: 'disable']();
		}
		if (this.contextMenu.items.get('expand')) {
			this.contextMenu.items.get('expand')[node.attributes.leaf
					? 'disable'
					: 'enable']();
		}
		if (this.contextMenu.items.get('collapse')) {
			this.contextMenu.items.get('collapse')[node.attributes.leaf
					? 'disable'
					: 'enable']();
		}
	...
	configInfo : function() {
		...
		this.dialog
				.show(this.getSelectionModel().getSelectedNode().ui.textNode);
		this.formPanel.load({
	...
	onDestroy : function() {
		...
		JsonTree.superclass.onDestroy.call(this);
	}
});
Ext.reg('JsonTree', JsonTree);