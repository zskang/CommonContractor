...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,
		Ext.form.ComboBox, {
			...
			setMode : function(singleMode) {
				//如果从多选变成单选，则清空选项。
				if (singleMode != this.singleMode && singleMode
						&& this.items.length > 1)
					this.removeAllItems();
			...
			onStoreLoad : function(store, records, options) {
				...
				var q = options.params[this.queryParam]
						|| store.baseParams[this.queryParam] || "", isValuesQuery = options.params[this.queryValuesIndicator]
			...
			doTransform : function() {
				...
				if (!this.store) {
					...
					for (var i = 0, len = opts.length; i < len; i++) {
						var o = opts[i], oe = Ext.get(o), value = oe
								.getAttributeNS(null, 'value')
								|| '', cls = oe.getAttributeNS(null,
								'className')
			...
			manageNameAttribute : function() {
				if (this.items.getCount() === 0 && this.forceFormValue) {
					if (!Ext.isIE6) {
						this.el.dom.setAttribute('name', this.hiddenName
										|| this.name);
					} else {
						this.el.dom.setAttribute('name', this.hiddenName);
					}
			...
			onKeyUp : function(e) {
				if (this.editable !== false
						&& (!e.isSpecialKey() || e.getKey() === e.BACKSPACE)
						&& e.getKey() !== this.itemDelimiterKey
